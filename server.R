function(input, output, session) {
  ...
  #### http://127.0.0.1:3028/?memberId=e6cfddcb-f8a4-452a-bafc-54323acb38e6
  #### http://127.0.0.1:3775/?memberId=1b972499-5bc7-422c-b1c6-57a91ec9fde2
  observe({
  ...
  observeEvent(users$memberId, {
    if (is.na(users$memberId) | users$memberId == "") {
      return()
    }
    ...
    withProgress(
      {
        ...
        stock$data <- dbGetQuery(conn = con(), glue("SELECT * FROM stock WHERE id_kund = {clients$selected$id}")) |>
          select(-id) |>
          mutate(
            id_kund = as.integer(clients$selected$id),
